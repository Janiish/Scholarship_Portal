    ...
    <div className="page-enter py-12 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8 slide-up">
          ...
          <h1 className="text-2xl font-bold text-white text-glow">Track Your Applications</h1>
          <p className="text-slate-400 mt-1">Enter your email to view all submitted applications.</p>
        </div>
        ...
        <div className="flex gap-3 mb-8 slide-up delay-200">
          <input
            ...
            className="input-glass flex-1 px-4 py-3 rounded-xl"
          />
          <button
            onClick={handleCheck}
            disabled={loading}
            className="btn-glow ripple px-6 py-3 text-white font-semibold rounded-xl disabled:opacity-50 whitespace-nowrap"
          >
        ...
        {/* Error */}
        {error && (
          <div className="mb-6 p-4 rounded-xl badge-rejected text-sm font-medium text-center scale-in">
            {error}
          </div>
        )}
        ...
        {applications && applications.length === 0 && (
          <div className="glass rounded-2xl p-12 text-center scale-in">
            <HiOutlineInboxStack className="w-12 h-12 text-slate-600 mx-auto mb-4" />
            <p className="text-slate-300 font-medium">No applications found for this email.</p>
            <p className="text-slate-500 text-sm mt-1">Make sure you entered the correct email address.</p>
          </div>
        )}
        ...
        {applications && applications.length > 0 && (
          <div className="space-y-4">
            ...
            {applications.map((app, i) => {
              ...
              return (
                <div key={app._id} className={`glass rounded-2xl p-5 flex items-center justify-between hover-lift hover-shine group slide-up`} style={{ animationDelay: `${i * 100}ms` }}>
                  <div>
                    <h3 className="text-lg font-semibold text-white group-hover-slide">{app.scholarshipName}</h3>
                    <p className="text-sm text-slate-500 mt-1">
                      Applied on {new Date(app.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                    </p>
                    {app.gpa && <p className="text-xs text-slate-500 mt-0.5">GPA: {app.gpa}</p>}
                  </div>
